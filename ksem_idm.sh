...
if [[ $output_idm == *"Written 1 references"* ]]; then
  echo "$now - successfully send $fed_in_energy to $ip_idm"
else
  echo "$now - failed sending $fed_in_energy to $ip_idm"
  echo $output_idm
fi

# as GUI in iDM is not showing correct values, we will reed and print
...
# print it out
now=$(date +"%d.%m.%Y %T")
echo "$now - read value iDM pump (kW): " $output_idm_energy
...
# print it out
now=$(date +"%d.%m.%Y %T")
echo "$now - read load iDM pump (kW): " $output_idm_energy
